...
const MaxWidthColumn: React.FC<buttonprops> = ({ width, children }) => (
...
export const useTable = () => {
  ...
  const [dependences, setDependences] = useState<IDependences>({ cidades: [], addons: [] });
  ...
  const filterKey = ['Ativos', 'Lixeira'];
  ...
  const columns: ColumnsType<IRowData> = [
    {
      title: 'Nome',
      dataIndex: 'nome',
      render: (_, row: IRowData) => <MaxWidthColumn width={180}>{row.nome}</MaxWidthColumn>,
      ...defaultColumnProps,
    },
    {
      title: 'Estado',
      dataIndex: 'uf',
      render: (_, row: IRowData) => <MaxWidthColumn width={180}>{row.uf}</MaxWidthColumn>,
      ...defaultColumnProps,
    },
    {
      title: 'Sigla',
      dataIndex: 'sigla',
      render: (_, row: IRowData) => <MaxWidthColumn width={180}>{row.sigla}</MaxWidthColumn>,
      ...defaultColumnProps,
    },
...

    {
      title: 'Adicionado Em',
      dataIndex: 'createdAt',
      render: (_, row: IRowData) => (row.createdAt ? moment(row.createdAt).format('DD/MM/YYYY') : '---'),
      ...defaultColumnProps,
    },
