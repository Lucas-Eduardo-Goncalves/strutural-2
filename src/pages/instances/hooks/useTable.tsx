...
export const useTable = () => {
  ...
  const columns: ColumnsType<IRowData> = [
    ...
    },
    {
      title: 'Número',
      dataIndex: 'instance',
      render: (_, row: IRowData) => <MaxWidthColumn width={180}>{row.instance}</MaxWidthColumn>,
      ...defaultColumnProps,
    },
  ...
  
    {
      ...
      render: (_, row) => (
        <div className="table-actions">
          <Button className="btn-icon" type="info" onClick={()=> handleOpenModalQRCode(row.instance)} shape="circle">
            Ativar
          </Button>
          ...
          {statusSelected === 'Inativos' ? (
            ...
            <>
              <Button className="btn-icon" type="info" onClick={()=> window.open(row.urlApi, "_blank")} to={row.urlApi} shape="circle">
  ...
  const [userQRInstanseID, setUserQRInstanseID] = useState("");

  function handleOpenModalQRCode(id: string) {
    setUserQRInstanseID(id);
